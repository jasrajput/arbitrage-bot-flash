    ...
    for (let i = 0; i < tokenList.length; i++) {
        for (let j = 0; j < tokenList.length; j++) {
            if (i !== j) {
                ...
                if (token1.usd && token2.usd) {
                    // Calculate exchange rates
                    const rate1 = parseFloat(token2.usd) / parseFloat(token1.usd);
                    const rate2 = parseFloat(token1.usd) / parseFloat(token2.usd);

                    // Calculate arbitrage ratio for the two-step conversion
                    let arbitrageRatio = rate1 * rate2;
                    ...
                    const slippageFee = investmentAmount * SLLIPAGE;
                    ...
                    const netProfit = investmentAmount * (arbitrageRatio - 1) - aaveNetworkFee - FIXED_NETWORK_FEE - slippageFee - exchangeFee + randomOffset;

                    opportunities.push({
                        prices: [token1.usd, token2.usd],
                        tokens: [token1.symbol, token2.symbol],
        ...
        {arbitrageOpportunities.slice(0, displayCount).map((opportunity, index) => (
          <li key={index} className="border p-4 rounded-lg shadow bg-white">
            <p className="font-semibold text-lg mb-2">
              {opportunity.tokens[0]} → {opportunity.tokens[1]} → {opportunity.tokens[0]}
            </p>
